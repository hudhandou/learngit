...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		...
		<div class="container" style="margin-top:10px;">
			<form action="" class="form form-horizontal" method="post">
				<div class="form-group">
					<label class="control-label col-sm-2 col-md-2">学科大类别名:</label>
					<div class="col-sm-3">
						<input class="form-control" name="subjectName" value="${subjectBigPage.subjectName}" placeholder="请输入学科大类别名称" />
					</div>
					<div class="col-sm-1">
		...
		<div class="container text-center"  >
			<page:mypage3 pageName="pageInfo" uri="/subjectBig/query.action"></page:mypage3>
		</div>
		<%-- 分页end --%>
	</body>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/big/addPage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/big/editPage.action?id="+id;
	}
	
	function moren(id){
		window.location.href = "<%=ctx%>/big/moren.action?id="+id;
	}
	//全选
	function selectAll(){
	...
	function del(){	
		...
		bootbox.confirm("确定要删除选中的记录吗?",function(r){
			if(r){
				...
				$.ajax({
					type: "post", //请求方式
					url :"<%=ctx%>/big/delete.action",  //请求路径
