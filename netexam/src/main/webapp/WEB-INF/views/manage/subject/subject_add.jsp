...
<body style="background: linear-gradient(white, #ebebeb, white);">
	...
	<div class="container">
		<hr>
		<form class="form-horizontal text-center" name="infoForm"
			action="javascript:doSub()" method="post">
			...
			<div class="container ">
				<div class="form-group">
					...
					<div class="col-md-9 col-sm-9">
						<select name="subjectPid" class="form-control">
							...
							<c:forEach items="${sjb}" var="sjb">
								<option value="${sjb.subjectId}">${sjb.subjectName}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-md-9 col-sm-9">
						<input class="form-control" noinput="subjectname1(this.value)"
							name="subjectName" value="" placeholder="请输入学科名称" onblur="blurckName(this.value)"/>
					</div>
				</div>
				<div class="form-group">
	...
	function doSub(){
	 ...
	 bootbox.confirm("确认提交吗?",function(r){
		 if(r){
			 $.ajax({
				type:"POST",
				url:"<%=ctx%>/subject/addSave.action",
